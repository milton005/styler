...
import com.richie.styler.UI.Models.Likes.Like;
import com.richie.styler.UI.Models.Likes.ModelMyLikes;
import com.richie.styler.UI.Models.Likes.Result;
...
//                intent.putExtra("distance",distance);
    ...
    private void getWebService() {
        ...
        APIService service = ServiceGenerator.createService(APIService.class, LIkeActivity.this);
        Call<ModelMyLikes> call=service.getmylikes(params);
        call.enqueue(new Callback<ModelMyLikes>() {
            @Override
            public void onResponse(Call<ModelMyLikes> call, Response<ModelMyLikes> response) {
                if (response.isSuccessful()) {
//                    showToast("Success",true);
                    hideProgress();
                    if (response.body() != null && response.body().getResult() != null) {
                        ModelMyLikes modelMyfavorite=response.body();
                        Result result=modelMyfavorite.getResult();
                        newStylers=result.getLikes();
                        list1.clear();
            ...
            @Override
            public void onFailure(Call<ModelMyLikes> call, Throwable t) {

            }
        });
    }
    private void setadapter() {
        adpter = new GRIDHOMEADAPTER(LIkeActivity.this, list1);
