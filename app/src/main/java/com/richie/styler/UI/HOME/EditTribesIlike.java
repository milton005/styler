...
import com.richie.styler.UI.Models.Profile.TribesLookingFor;
...
public class EditTribesIlike extends BaseActivity {
...
//    SpinnerAdapter adapter;
    ...
    List<Tribe>tribes;
    List<TribesLookingFor>tribesLookingFors;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tribesLookingFors=new ArrayList<>();
        list3=new ArrayList<>();
        gestureDetector = new GestureDetector( new EditTribesIlike.SwipeDetector() );
                    ...
                    if (enlarged.getVisibility() == View.VISIBLE) {
                        ...
                        if (pos != gridItems.size() - 1) {
                            ...
                            if (!gridItems.get(pos).getTribeImage().isEmpty())
                            {
                                ...
                                Picasso.with(EditTribesIlike.this).load(gridItems.get(pos).getTribeImage()).into(image_enlarged);
                                style.setText(gridItems.get(pos).getTribeName());
                                int dotsCount=gridItems.size();
                                dots = new ImageView[dotsCount];
                                pagerindicator.removeAllViews();
                                for (int i = 0; i < dotsCount; i++) {
                                    dots[i] = new ImageView(EditTribesIlike.this);
...
//        showProgress();
        ...
        APIService service= ServiceGenerator.createService(APIService.class,EditTribesIlike.this);
...
//                    hideProgress();
                    if (response.body()!=null)
                    {
                        ...
                        tribesLookingFors=profile.getUser().get(0).getTribesLookingFor();
                        if (tribesLookingFors!=null) {
                            for (int i = 0; i < tribesLookingFors.size(); i++) {
                                String tribetitle = tribesLookingFors.get(i).getTribeId();
                                for (int j = 0; j < gridItems.size(); j++) {
                                    if (gridItems.get(j).getUserid().equals(tribetitle)) {
...
//                        if (gender.equals("m")) {
                            ...
                            for (int i = 0; i < list1.size(); i++) {
                                ...
                                model.setUserid(list1.get(i).getTribeId());
                                gridItems.add(model);
...
//                            gridItems.clear();
                            for (int i = 0; i < list2.size(); i++) {
                                ...
                                model.setUserid(list2.get(i).getTribeId());
                                gridItems.add(model);
                                Listing l=new Listing();
            ...
            if (resultCode==Activity.RESULT_OK)
            {
                try {
                    ...
                    if (edit)
                    {
                        ...
                        Intent intent= new Intent(EditTribesIlike.this,PopUpAddTribe.class);
            ...
            public void onItemClicked(final RecyclerView recyclerView, final int position, View v) {
                if (gridItems.get(position).isSelected())
                {
                    ...
                    if (!gridItems.get(position).getTribeImage().isEmpty()) {
                        ...
                        dots[pos].setImageDrawable(getResources().getDrawable(R.drawable.selected_itemdot));
                        tribelayout.setBackgroundColor(getResources().getColor(R.color.lavender));
                        Picasso.with(EditTribesIlike.this).load(gridItems.get(position).getTribeImage()).into(image_enlarged);
                        ...
                        closeButton.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                style.setText(R.string.select_tribes_ilike);
                                recyclerView.setVisibility(View.VISIBLE);
                                other.setVisibility(View.VISIBLE);
                                tribelayout.setBackgroundColor(Color.TRANSPARENT);
                                enlarged.setVisibility(View.GONE);
                        ...
                        Intent intent=new Intent(EditTribesIlike.this,PopupdeleteTribe.class);
                        intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
                        intent.putExtra("position",position);
...
//            progressDialog.show();
            ...
            String userid = getPref(Constants.USER_ID);
            params.put("user_id", userid);
            params.put("tribes_looking_for", build);
            if (!OTHERTRIBENAME.isEmpty())
            {
                params.put("other_tribe",otherbuild.toString());
            }
            final APIService service = ServiceGenerator.createService(APIService.class, EditTribesIlike.this);
