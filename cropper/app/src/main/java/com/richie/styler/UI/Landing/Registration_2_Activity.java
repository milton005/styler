...
RecyclerView.LayoutManager mLayoutManager;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gestureDetector = new GestureDetector( new Registration_2_Activity.SwipeDetector() );
        setAdapter();
                ...
                if (gridItems.get(position).isSelected())
                {
                    ...
                    if (!gridItems.get(position).getTribeImage().isEmpty()) {
                        ...
                        for (int i = 0; i < dotsCount; i++) {
                            dots[i] = new ImageView(Registration_2_Activity.this);
                ...
                if (gridItems.size()>1) {
                    if (enlarged.getVisibility() == View.VISIBLE) {
                        ...
                        if (pos != gridItems.size() - 1) {
                            ...
                            {
                                tribelayout.setBackgroundColor(getResources().getColor(R.color.lavender));
                                Picasso.with(Registration_2_Activity.this).load(gridItems.get(pos).getTribeImage()).into(image_enlarged);
    ...
    private void getWebservice(final String build) {
        if (!build.isEmpty()) {
            ...
            for (int i=0;i<gridItems.size();i++)
            {
                if (gridItems.get(i).getTribeImage().isEmpty())
                {
                    String s=gridItems.get(i).getTribeName();
                    otherbuild.append(s);
                    otherbuild.append(",");
                }
            }
